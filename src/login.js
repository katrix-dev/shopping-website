	...
	const GoLogin = () =>{
		...
		.then(response=>{
			for(var i=0; i<response.data.length; i++){
				var vendor = response.data[i]; // assign one vendor info to vendor variable
                ...
                <div class="bg-light rounded p-3">
					...
					<p class="alert alert-danger text-center"> {msg} </p>
						<div class="mb-3 input-group">
							...
							<input type="email" class="form-control" placeholder="Enter Email" onChange={obj=>pickEmail(obj.target.value)}/>
						</div>
						<div class="mb-3 input-group">
                            <span class="input-group-text">
								<i class="fa fa-lock"></i>
							</span>	
							<input type="password" class="form-control" placeholder="Enter Password" onChange={obj=>pickPassword(obj.target.value)}/>
							
						</div>
						<div class="text-center">
							<button class="btn btn-primary" onClick={GoLogin}>Login <i class="fa fa-arrow-right"></i> </button>
							<hr/>
						</div>
					</div>
